...
// ============================================================
// TELEFON İLƏ GİRİŞ — reCAPTCHA + SMS
// ============================================================

// ── Görünməz reCAPTCHA-nı (yenidən) qur ───────────────────────
function _initRecaptcha() {
  if (!_auth || !_fbReady()) return null;
...
// ── 1-ci addım: nömrə daxil et, kodu göndər ───────────────────
async function sendPhoneCode() {
  ...
  if (digits.length !== 9) {
    _showAuthError('Telefon nömrəsini düzgün daxil et (9 rəqəm, məs. 50 123 45 67).');
    return;
  }

  const phoneNumber = '+994' + digits;
  const btn = document.getElementById('phone-send-btn');
  if (btn) { btn.disabled = true; btn.textContent = 'Göndərilir...'; }

  try {
    const verifier = _initRecaptcha();
    const confirmationResult = await _auth.signInWithPhoneNumber(phoneNumber, verifier);
    ...
    console.error('[firebase] SMS göndərmə xətası:', e);
    _showAuthError(_authErrorMessage(e.code));
    _initRecaptcha(); // növbəti cəhd üçün təzədən qur
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = 'Kodu göndər'; }
  }
}

// ── 2-ci addım: SMS kodunu yoxla ──────────────────────────────
async function verifyPhoneCode() {
  const code = document.getElementById('phone-code-input')?.value?.trim() || '';
  if (code.length < 6) {
    _showAuthError('6 rəqəmli kodu daxil et.');
    return;
  }
  if (!window._phoneConfirmation) {
    _showAuthError('Əvvəlcə kodu göndər.');
    return;
  }

  const btn = document.getElementById('phone-verify-btn');
  if (btn) { btn.disabled = true; btn.textContent = 'Yoxlanılır...'; }

  try {
    await window._phoneConfirmation.confirm(code);
    // Uğurlu olarsa onAuthStateChange modalı özü bağlayacaq
  } catch (e) {
    console.error('[firebase] Kod yoxlama xətası:', e);
    _showAuthError(_authErrorMessage(e.code) || 'Kod yanlışdır və ya vaxtı keçib.');
  } finally {
    if (btn) { btn.disabled = false; btn.textContent = 'Təsdiqlə'; }
  }
}

// ── Nömrə addımına geri qayıt ─────────────────────────────────
function resetPhoneStep() {
  ...
  window._phoneConfirmation = null;
}
...
function _authErrorMessage(code) {
  const map = {
    ...
    'auth/invalid-phone-number':      'Telefon nömrəsi düzgün formatda deyil.',
    'auth/missing-phone-number':      'Telefon nömrəsini daxil et.',
    'auth/invalid-verification-code': 'Kod yanlışdır.',
    'auth/code-expired':              'Kodun vaxtı keçib. Yenidən göndər.',
    'auth/captcha-check-failed':      'ReCAPTCHA doğrulaması alınmadı. Səhifəni yeniləyib yenidən cəhd et.',
    'auth/quota-exceeded':            'SMS limiti aşıldı. Bir az sonra yenidən cəhd et.'
  };
...
function setAuthMode(mode) {
  ...
  if (mode === 'phone' && typeof resetPhoneStep === 'function') resetPhoneStep();
}
...
async function showMonthlyVisitors() {
  ...
  try {
    ...
    // Köhnə gizli element (uyğunluq üçün)
    const legacyEl = document.getElementById('stat-monthly-visitors');
    if (legacyEl) legacyEl.textContent = count.toLocaleString('az-AZ');

    // Yeni böyük stat kart — sc-students
    const scEl = document.getElementById('sc-students');
    if (scEl && count > 0) {
      // data-target-i real Firebase dəyəri ilə yenilə
      scEl.setAttribute('data-target', count);

      // Counter hələ 0-dursa → animated sayma işə düşsün
      if (scEl.textContent === '0' || scEl.textContent === '') {
        _animateCounter(scEl, count, 1800);
      } else {
        // Counter artıq işə düşüb (IntersectionObserver tez tetiklənib) — dərhal real dəyəri yaz
        scEl.textContent = count.toLocaleString('az-AZ') + '+';
      }
    }
  } catch (e) {
...
// ── Shared counter helper (firebase.js-dən çağırıla bilər) ───
function _animateCounter(el, target, duration) {
  ...
  function step(ts) {
    ...
    el.textContent = Math.floor(ease * target).toLocaleString('az-AZ');
